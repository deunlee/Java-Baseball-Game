...
- 1부터 9까지 서로 다른 수로 이루어진 3자리의 수를 맞추는 게임입니다.
  ...
  * 다른 자리에 있으면 `볼`
  * 같은 수가 전혀 없으면 `낫싱`이란 힌트를 얻습니다.
- 힌트를 이용해서 먼저 상대방의 수를 맞추면 승리합니다.
- 상대방의 역할은 컴퓨터가 합니다.
  * 컴퓨터는 1에서 9까지 서로 다른 임의의 수 3개를 선택합니다.
  * 플레이어는 컴퓨터가 생각한 3개의 숫자를 입력, 컴퓨터는 입력한 숫자에 대한 결과를 출력합니다.
  * 이 과정을 반복해 컴퓨터가 선택한 3개의 숫자를 모두 맞히면 게임이 종료됩니다.
- 게임을 종료한 후 게임을 다시 시작하거나 완전히 종료할 수 있습니다.
- 사용자가 잘못된 값을 입력할 경우 `[ERROR]`로 시작하는 메시지를 출력하고, 게임을 계속 진행할 수 있어야 합니다.
...
- 메서드의 길이가 15라인을 넘어가지 않도록 구현 (한 가지 일만 잘 하도록 구현)
